...
{  
	...
	$options[] = array(
		'name' => '产品显示列数和个数',
        'type' => 'title',
        'desc' => '通过下方选项调整产品列表页中每一行显示的列数和每页显示的产品数量.<br/><small>注意：该选项由<strong>'.THEMENAME.' 主题</strong>添加，切换主题后不会显示</small>',
        'id'   => 'column_options'
	);
...
function avia_woocommerce_product_elements($elements)
{
    ...
    if(!empty($posttype) && $posttype == 'product')
    {
        $elements[] = array("slug"	=> "avia_product_hover",
            "name" 	=> "<strong>产品列表</strong>鼠标悬停效果",
            "desc" 	=> "选择是否在产品列表页显示一个鼠标指向产品时图片切换的效果，切换的图片为产品相册的第一张.",
            "id" 	=> "_product_hover",
            "type" 	=> "select",
            "std" 	=> "",
            "class" => "avia-style",
            "subtype" => array("显示，指向时以产品相册第一张替换" => 'hover_active', "不显示" => ''));
