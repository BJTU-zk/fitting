	...
	class avia_sc_video extends aviaShortcodeTemplate
	{
			...
			function popup_elements()
			{
				$this->elements = array(
					...
					// 5Usujian
					array(	
							"name" 	=> __("视频封面图片",'avia_framework' ),
							"desc" 	=> __("插入或上传一张视频封面图片",'avia_framework' )."<br/><br/>",
							
							"id" 	=> "poster",
							"type" 	=> "image",
							"title" => __("插入图片",'avia_framework' ),
							"button" => __("插入",'avia_framework' ),
							"std" 	=> ""),
					array(	
			...
			function shortcode_handler($atts, $content = "", $shortcodename = "", $meta = "")
			{
				...
				// 5Usujian
				extract(shortcode_atts(array('src' => '', 'src_1' => '', 'src_2' => '', 'autoplay' => '', 'format' => '', 'poster' => '', 'height'=>'9', 'width'=>'16'), $atts, $this->config['shortcode']));
                ...
                else
                {
                    ...
                    if(in_array($file_extension, array('ogv','webm','mp4'))) //check for html 5 video
                    {
                    	...
                    	if ($poster != '') {
                    		...
                    		$output = avia_html5_video_embed($src, $poster);
                    	}else{
