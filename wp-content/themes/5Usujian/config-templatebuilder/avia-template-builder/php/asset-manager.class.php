	...
	class aviaAssetManager
	{
		...
		public function merge( $file_type , $prefix , $conditions = array())
		{
			...
			if($file_exists && $file_exists !== "error-generating-file")
			{
				...
				$avia_dyn_stylesheet_url = $avia_upload_dir['baseurl'] . '/dynamic_avia/'.$data['hash'].'.'.$file_type;
				
				//if file exists enque it
				if($file_type == 'css')
				{
					wp_enqueue_style( $prefix , $avia_dyn_stylesheet_url, array(), false, 'all' );
				}
				else
				{
					$footer = isset($conditions['groups']) ? $conditions['groups'] : true;
					
					wp_enqueue_script( $prefix , $avia_dyn_stylesheet_url, array(), false, $footer );
				}
			}
