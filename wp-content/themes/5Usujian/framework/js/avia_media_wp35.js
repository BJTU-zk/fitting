...
*************************************************************************/
	...
	$.AviaElementBehavior.fontello_insert = function(event, selection, options)
	{
		...
		{
			$('body').avia_alert({the_class:'error', text:'请上传一个可用的zip压缩文件.<br/>你可以在这里获取文件：Fontello.com'});
			return;
		}
		...
		$.ajax({
			...
			error: function()
			{
				$('body').avia_alert({the_class:'error', text:'无法添加字体，因为服务器无响应.<br/>请刷新页面后重试。'});
			},
			success: function(response)
			{
				...
				{
					$('body').avia_alert({the_class:'error', show:6500 , text:'无法添加字体。<br/>发生如下错误: '+"<br/><br/>"+response});
				}
	...
	$.AviaElementBehavior.fontello_remove = function(event)
	{
		...
		$.ajax({
			...
			error: function()
			{
				$('body').avia_alert({the_class:'error', text:'无法删除字体，因为服务器无响应.<br/>请刷新页面后重试。'});
			},
			success: function(response)
			{
				...
				{
					$('body').avia_alert({the_class:'error', text:'无法删除字体.<br/>请刷新页面后重试。'});
				}
    ...
    $.AviaElementBehavior.config_file_insert = function(event, selection, options)
    {
        ...
        {
            $('body').avia_alert({the_class:'error', text:'无法识别的配置文件.<br/>您可以点击“导出主题设置文件”按钮来创建配置文件。'});
            return;
        }
        ...
        $.ajax({
            ...
            error: function()
            {
                $('body').avia_alert({the_class:'error', text:'导入失败，因为服务器无响应，请刷新页面后重新尝试。'});
            },
            success: function(response)
            {
                if(response.match(/avia_config_file_imported/))
                {
					$('body').avia_alert({text: '导入成功！<br/>一切顺利，未发生任何错误。 <br/>页面即将刷新给您展示所作更改'}, function()
								{
                ...
                {
                    $('body').avia_alert({the_class:'error', show:6500 , text:'导入失败<br/>发生如下错误:  '+"<br/><br/>"+response});
                }
