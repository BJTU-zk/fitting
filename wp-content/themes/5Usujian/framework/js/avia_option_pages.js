...
(function($)
{
	...
	var	methods = {
		...
		save: function(passed, hiddensave)
		{
			...
			$.ajax({
					...
					{	
						...
						//allow saving again
						$('body').avia_alert({the_class:'error', text:'保存失败！ <br/> 请刷新页面后重试', show:4500});
					...
					{	
						...
						else
						{
							...
							{
								answer = '保存失败！ <br/>以下为返回错误信息： <br/><br/>'+response;
							}
							else
							{
								answer = '保存失败！ <br/>请刷新页面后重试';
							}
		...
		do_parent_import: function(passed)
		{
			...
			activate = confirm('导入示例数据会覆盖您当前主题的设置，确定导入？')
			...
			$.ajax({
						...
						success: function(response)
						{
							...
							else
							{
								...
								$('body').avia_alert({	the_class:'error', 
														text:'导入失败！<br/>请尝试刷新页面并重新导入。<br/> (返回错误信息: <br/><br/>'+response+')', 
														show:4500});
							}
						},
						complete: function(response)
		...
		do_import: function(passed)
		{
			...
			var button = $(this),
				...
				answer = "",
				activate = true,
				message = "导入示例演示数据会覆盖您当前主题的设置并删除所有用模板编辑器制作的自定义模板，确定导入？";
			
			
								
			if(button.is('.avia_button_inactive')) return false;
			if(button.is('.avia_import_image')) message = "导入示例数据会覆盖您当前主题的设置，确定导入？";
		...
		reset: function(passed)
		{
			var me = passed.data.set,
				answer = confirm("该操作会删除主题到当前状态所有的设置并将主题选项恢复到出厂状态。 \n确定继续操做？ ");
			...
			if(answer)
			{
				$.ajax({
						...
						{
							//allow saving again
							$('body').avia_alert({the_class:'error', text:'重置失败！ <br/> 请稍等几秒后重试。', show:4500});
						...
						success: function(response)
						{
							...
							{	
								...
								if(response.length > 3)
								{
									answer = '重置失败！ <br/>返回错误信息为: <br/><br/>'+response;
								}
								else
								{
									answer = '重置失败！ <br/> 请稍等几秒后重试。';
								}
