...
if (!class_exists('avia_newsbox'))
{
	class avia_newsbox extends WP_Widget {
		...
		function __construct()
		{
			...
			parent::__construct( 'newsbox', THEMENAME.' 最近新闻', $widget_ops );
		}
...
if (!class_exists('avia_portfoliobox'))
{
	class avia_portfoliobox extends avia_newsbox
	{
		function __construct()
		{
			...
			WP_Widget::__construct( 'portfoliobox', THEMENAME.' 最近作品', $widget_ops );
		}
	}
}
...
if (!class_exists('avia_partner_widget'))
{
	class avia_partner_widget extends WP_Widget {
		...
		function __construct() {
			...
			parent::__construct( 'avia_partner_widget', THEMENAME.' 广告区域', $widget_ops );
		}
...
if (!class_exists('avia_combo_widget'))
{
	class avia_combo_widget extends WP_Widget {
		...
		function __construct() {
			//Constructor
			$widget_ops = array('classname' => 'avia_combo_widget', 'description' => __('一个可同时显示热门文章、最新文章、最新评论和标签云的小工具', 'avia_framework') );
			parent::__construct( 'avia_combo_widget', THEMENAME.' 组合小工具', $widget_ops );
		}
		...
		function widget($args, $instance)
		{
			...
			echo '<div class="tab first_tab active_tab widget_tab_popular"><span>'.__('热门', 'avia_framework').'</span></div>';
			echo "<div class='tab_content active_tab_content'>";
			avia_get_post_list('cat=&orderby=comment_count&posts_per_page='.$posts);
			echo "</div>";

			echo '<div class="tab widget_tab_recent"><span>'.__('最新', 'avia_framework').'</span></div>';
			echo "<div class='tab_content'>";
			avia_get_post_list('showposts='. $posts .'&orderby=post_date&order=desc');
			echo "</div>";

			echo '<div class="tab widget_tab_comments"><span>'.__('评论', 'avia_framework').'</span></div>';
			echo "<div class='tab_content'>";
			avia_get_comment_list( array('number' => $posts, 'status' => 'approve', 'order' => 'DESC') );
			echo "</div>";

			echo '<div class="tab last_tab widget_tab_tags"><span>'.__('标签', 'avia_framework').'</span></div>';
...
if (!class_exists('avia_google_maps'))
{
	class avia_google_maps extends WP_Widget {
		...
		function __construct() {
			...
			parent::__construct('avia_google_maps', THEMENAME.' 谷歌地图小部件', $widget_ops);
