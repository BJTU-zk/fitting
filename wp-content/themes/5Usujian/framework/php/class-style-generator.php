...
if( !class_exists( 'avia_style_generator' ) )
{
	...
	class avia_style_generator
	{
			...
			foreach($this->stylewizardIDs as $id)
			{
				...
				foreach($options as $style)
				{
					...
					foreach($selectorArray as $selector => $ruleset)
					{
							...
							if($value != "" && $value != "true" && $value != "disabled" && $key != "id")
							{
								...
								else
								{
									...
									switch($key)
									{
										...
										if ($font_family == 'Microsoft-Yahei-websave') {
						...
						if(!empty($rules))
						{
							if ($rules == "font-family: Microsoft-Yahei-websave;") {
											$rules = 'font-family:"Microsoft Yahei";';
										}
		...
		{	
			//5usujian
			if(empty($this->google_fontlist) || $this->google_fontlist == "Microsoft-Yahei-websave"|| $this->google_fontlist == "Microsoft+Yahei" || $this->google_fontlist == "Simsun-websave") return;
			$this->google_fontlist = str_replace("Microsoft-Yahei-websave",'',$this->google_fontlist);
			$this->google_fontlist = str_replace("Simsun-websave",'',$this->google_fontlist);
