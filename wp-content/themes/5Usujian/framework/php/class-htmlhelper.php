...
if( ! class_exists( 'avia_htmlhelper' ) )
{
	class avia_htmlhelper
	{
		...
		{	
			...
			if(empty($element['button-label'])) $element['button-label'] = "上传";
		...
		{	
			...
			$output .= '<div class="avia_import_wait"><strong>正在安装.</strong><br/>请稍等片刻，不要刷新或关闭页面，网站很快就会自动完成安装~(通常需要几分钟) :)</div>';
     ...
     */
        function parent_setting_import($element)
        {
            ...
            $output .= '<div class="avia_import_parent_wait"><strong>正在安装.</strong><br/>请稍等片刻，不要刷新或关闭页面，网站很快就会自动完成安装~(通常需要几分钟) :)</div>';
		...
		function page_footer()
		{
			...
			//5Usujian
			$output .= "<script type='text/javascript' src='".get_template_directory_uri()."/update/update.js'></script>";
			return $output;
		}
		...
		{
			$output = '<span class="avia_style_wrap"><a href="#" class="avia_button avia_button_inactive avia_submit">保存所有</a></span>';
			return $output;
		}
		...
		function reset_button()
		{
			...
			$output = '<span class="avia_style_wrap"><a href="#" class="avia_button avia_button_grey avia_reset">重置所有</a></span>';
			return $output;
		}
